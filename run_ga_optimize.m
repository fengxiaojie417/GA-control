%% run_ga_optimize.m - 遗传算法优化PID参数主程序 (论文版)
% 功能：使用遗传算法优化双容水箱PID控制器参数，生成高质量论文图表
...
if isfield(output_ga, 'bestfval')
    generations = 1:length(output_ga.bestfval);
    
    % 绘制收敛曲线
    hold on; box on; grid on;
    plot(generations, output_ga.bestfval, '-o', 'Color', color_red, ...
        'LineWidth', 2.5, 'MarkerSize', 4, 'MarkerFaceColor', color_red);
    ...
    xlim([1, length(generations)]);
    set(gca, 'LineWidth', 1.2);
    
    % 标注关键点
    [min_val, min_idx] = min(output_ga.bestfval);
    plot(min_idx, min_val, 'p', 'MarkerSize', 15, 'MarkerFaceColor', 'g', ...
        'MarkerEdgeColor', 'k', 'LineWidth', 2);
    text(min_idx, min_val*1.1, sprintf('最优解\n第%d代\nf=%.4f', min_idx, min_val), ...
...
if isfield(output_ga, 'bestfval')
    % 计算收敛速度
    convergence_rate = -diff(output_ga.bestfval);
    convergence_rate(convergence_rate < 0) = 0;  % 只看改善
    
    hold on; box on; grid on;
    bar(2:length(output_ga.bestfval), convergence_rate, ...
...
xlim([0, max(improvement_data)*1.15]);
ylim([0.5, 6.5]);
...
if isfield(output_ga, 'bestfval')
    hold on; box on; grid on;
    plot(1:length(output_ga.bestfval), output_ga.bestfval, '-o', ...
    ...
    xlim([1, length(output_ga.bestfval)]);
    set(gca, 'LineWidth', 1);
end
